...
const { retryFunction } = require('../helpers/retry')
...
module.exports.handler = async (event) => {
  ...
  const saveStationsToDb = await station.saveToDb(stations, client)
  ...
  const saveStationsToS3 = await station.saveToObjects(stations, bucket, s3)
  await client.end()

  return { saveStationsToDb, saveStationsToS3 }
}
