...
async function getImtdApiResponse (stationId) {
  ...
  try {
    ...
    if (error.response.status === 404) {
      logger.info(`Station ${stationId} not found in IMTD (HTTP Status: 404)`)
      return {}
    } else {
      throw new Error(`Request to IMTD API failed for RLOI id ${stationId} (status: ${error.response.status})`)
    }
  }
}
