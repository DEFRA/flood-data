...
async function getFile (index) {
  ...
  if (index >= fileNames.length) {
    ...
    return Promise.reject(new Error('None of the files were found in the S3 bucket'))
  }
  ...
  try {
    ...
    if (data === undefined) {
      return Promise.reject(new Error(`File ${fileName} not found in the S3 bucket`))
    }
    // Process data here
    return data
  } catch (err) {
