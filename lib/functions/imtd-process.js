...
module.exports.handler = async (event) => {
  ...
  fetchDataForStationIds(processIds)
    .then((results) => {
      ...
      const flattenedData = filtered.reduce(
...
async function updateStationsList(remainingIds) {
  ...
  try {
    ...
    if (err.code === 'NoSuchKey') {
      ...
      console.error(
        `Error deleting latest.json file from S3 bucket: ${err.message}`
      );
    }
  }
  ...
  try {
    ...
    if (err.code === 'NoSuchKey') {
      ...
      console.error(
        `Error deleting remainingIds.json file from S3 bucket: ${err.message}`
      );
    }
  }
  ...
  if (remainingIds.length > 0) {
    ...
    await s3.putObject({
      Bucket: bucket,
      Key: key,
      Body: JSON.stringify(remainingIds),
    });
