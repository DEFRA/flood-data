module.exports = {
  ...
  getAllStations: 'select distinct rloi_id from rivers_mview where rloi_id is not null order by rloi_id asc',
  deleteImtd: 'Truncate table u_flood.imtd_niki;',
  insertImtd: 'INSERT INTO imtd_niki (station_id, fwis_code, fwis_type, direction, value) SELECT $1, $2, $3, $4, $5 WHERE NOT EXISTS (SELECT 1 FROM imtd_niki WHERE station_id = $1 AND fwis_code = $2 AND fwis_type = $3 AND direction = $4 AND value = $5);',
  refreshImtdMview: 'refresh materialized view u_flood.imtd_thresholds_mview with data;'
}
