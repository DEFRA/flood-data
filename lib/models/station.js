...
const uploadStation = async (station, bucket, s3) => {
  const params = {
  ...
  try {
    ...
    console.error(`Failed to upload: ${params.Key}`, error)
    return { success: false, error, key: params.Key }
  }
}

module.exports = {
  async saveToUploads (stations, bucket, s3) {
    ...
    try {
      await s3.upload(stationsParams)
      console.log(`${stationsParams.Key} uploaded`)

      const results = await Promise.all(stations.map((data) => uploadStation(data, bucket, s3)))

      const failed = results.filter(result => !result.success)
      if (failed.length > 0) {
        console.error(`Failed to upload ${failed.length} station(s)`)
        failed.forEach(f => console.error(`Failed: ${f.key}`, f.error))
      } else {
        console.log('All stations processed')
      }
      return results
    } catch (error) {
      console.error(`Failed to upload: ${stationsParams.Key}`, error)
      throw error
    }
  },
  ...
  async saveToDb (stations, client) {
    ...
    let totalStatitonsProcessed = 0

    for (let i = 0; i <= stationsFactor; i++) {
      const result = await client.query(stationTable.insert(dbStations.slice(i * 500, (i * 500) + 500)).toQuery())

      totalStatitonsProcessed = totalStatitonsProcessed + result.rowCount
    }

    // refresh mviews data
    await this.refreshStationMview(client)

    return totalStatitonsProcessed
  },
