...
module.exports = {
  ...
  async saveToDb (stations, pool) {
    const dbStations = stations.map((station, index) => {
      const row = {
    ...
    for (let i = 0; i <= stationsFactor; i++) {
      ...
      try {
        ...
        console.error(`❌ Batch insert failed at index ${i * batchSize}`)
        console.error(`❌ Batch error: ${batchError.message}`)

        // Fallback: Try each row individually to pinpoint failure
        for (let j = 0; j < batch.length; j++) {
          const row = batch[j]
          try {
            await pool.query('insertStations', [row]) // still expects an array
          } catch (rowError) {
            console.error(`❌ Failed to insert row ${i * batchSize + j + 1}`)
            console.error('❌ Data:', JSON.stringify(row, null, 2))
            console.error('❌ Error:', rowError.message)
            // optionally log constraint detail if available
            if (rowError.detail) {
              console.error('❌ Detail:', rowError.detail)
            }
          }
        }

        // Optionally rethrow to crash the lambda
        throw batchError
      }
    }
