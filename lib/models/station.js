  ...
  async saveToDb (stations, pool) {
    ...
    for (let i = 0; i <= stationsFactor; i++) {
      ...
      try {
        ...
        console.error(`❌ Batch insert failed (batch index ${i}):`)
        console.error(`❌ Error: ${err.message}`)
        console.error(`❌ Full query text: ${pool.sql.insertStations?.text || 'unknown'}`)
        console.error('❌ First row in batch:', batch[0])
        console.error('❌ Example payload keys:', Object.keys(batch[0]))
        throw err // optional, but rethrow if you want Lambda to crash for visibility
      }
    }
