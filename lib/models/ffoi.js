  ...
  async save (file, bucket, key, s3, client) {
    ...
    const promisesClean = file.EATimeSeriesDataExchangeFormat.Station
      ...
      .flatMap(item => {
        ...
        const upsertFfoiMaxDate = `${max.$.date}T${max.$.time}Z`

        return [
          s3.upload(params).catch(error => console.error('Failed to upload', error)),
          client.query(queries.upsertFfoiMax, [
            item.$.stationReference, max._, upsertFfoiMaxDate, key, currentDate
          ]).catch(error => console.error('Failed to add to db', error))
        ]
      })
    const result = []
