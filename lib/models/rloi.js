...
function setSlsTelemetryValueItem (index, res, station, setOfValues) {
  const value = {
    ...
    value: Number.parseFloat(setOfValues.Value[index]._),
    processed_value: Number.parseFloat(setOfValues.Value[index]._),
  ...
  if (setOfValues.$.parameter === 'Water Level') {
    ...
    if (station.Post_Process.toLowerCase() === 'y' || station.Post_Process.toLowerCase() === 'yes') {
      value.processed_value = station.Subtract ? Number.parseFloat(util.toFixed(value.value - Number.parseFloat(station.Subtract), processedValueDecimalPlace)) : value.value
    }
...
function setSlsTelemetryParent (key, station, item, setOfValues) {
  return [
    ...
    Number.parseFloat(station.Subtract),
    Number.parseFloat(station.POR_Max_Value),
    station.Station_Type,
    Number.parseFloat(station.percentile_5),
