...
class Rloi {
  ...
  async getStation (region, stationReference, bucket) {
    const regionLookup = regions[region] ? regions[region] : region
    try {
      const station = await this.s3.getObject({ Bucket: bucket, Key: `rloi/${regionLookup}/${stationReference}/station.json` })
  ...
  async saveTelemetryValues (parentId, setOfValues, station) {
  ...
  save (value, bucket, key) {
    ...
    return new Promise((resolve, reject) => {
      value.EATimeSeriesDataExchangeFormat.Station.filter((item) => item.SetofValues).forEach(async (item) => {
        const station = await this.getStation(item.$.region, item.$.stationReference, bucket)
        ...
        item.SetofValues.forEach(async (setOfValues) => {
          try {
            ...
            await this.saveTelemetryValues(parentId, setOfValues, station)
            processed++
