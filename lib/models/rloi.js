...
module.exports = {
            ...
            try {
              ...
              if (parameters.indexOf(setOfValues.$.parameter) > -1 || (station && setOfValues.$.parameter === 'Water Level')) {
                ...
                if (!station) {
                  ...
                  await client.raw(queries.slsTelemetryStation, telemetryStation)
                }
                ...
                const res = await client.raw(parentQuery, parent)
                const values = []
              ...
              if (processed === valuesCount) {
                console.log(`all values processed (${processed})`)
                resolve()
              }
            } catch (err) {
              return reject(err)
  ...
  async deleteOld (client) {
    await client.raw(queries.deleteOldTelemetry)
  }
}
